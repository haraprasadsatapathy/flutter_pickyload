...
class _HomeTabState extends State<HomeTab> with WidgetsBindingObserver {
  ...
  void _showCancelTripDialog(BuildContext context, TripDetail tripDetail) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
            ...
            TextButton(
              onPressed: () {
                ...
                context.read<HomeTabBloc>().add(
                  CancelTripOffer(offerId: tripDetail.offerId),
                );
              },
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<HomeTabBloc, HomeTabState>(
      listener: (context, state) {
        if (state is HomeTabCancelSuccess) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
        ...
        if (state is HomeTabCancelError) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
            ...
            child: RefreshIndicator(
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  if (isLoading &&
                      state.tripDetails.isEmpty &&
                      state.confirmedTrips.isEmpty)
                        ...
                        delegate: SliverChildListDelegate([
                          if (state.tripDetails.isEmpty &&
                            ...
                            if (state.confirmedTrips.isNotEmpty) ...[
                              ...
                              ...state.confirmedTrips.map(
                                (trip) =>
                                    _buildConfirmedTripCard(context, trip),
                              ),
                              const SizedBox(height: 24),
                            ],
                            ...
                            if (state.vehicleCount > 0 &&
                                ...
                                state.isAvailableForLoad.toLowerCase() ==
                                    'yes') ...[
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                IconButton(
                                  ...
                                  color: Colors.grey,
                                  tooltip: 'Copy pickup address',
                                ),
                              ],
                            ),
                            const SizedBox(height: 20),
                            // Drop address with copy button
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Expanded(
                      ...
                      children: [
                        Expanded(
                          child: _buildPaymentItem(
                            ...
                            label: 'Advance',
                            value:
                                'Rs. ${trip.advanceAmount.toStringAsFixed(0)}',
                        ...
                        Expanded(
                          child: _buildPaymentItem(
                            ...
                            label: 'Due',
                            value:
                                'Rs. ${trip.amountDue.abs().toStringAsFixed(0)}',
                            color: trip.amountDue > 0
                                ? Colors.red.shade700
  ...
  Widget _buildTripCard(BuildContext context, TripDetail tripDetail) {
    final bool isDriverOffered =
        tripDetail.tripStatus.toLowerCase() == 'driveroffered';
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () =>
                          _showCancelTripDialog(context, tripDetail),
  ...
  Widget _buildMyDocumentsSection(BuildContext context, HomeTabState state) {
    ...
    return Column(
      ...
      children: [
        ...
        if (!hasRC) _buildRCWarningCard(context),

        if (state.documents.isNotEmpty) ...[
          if (!hasRC) const SizedBox(height: 12),
          ...state.documents.map(
            (doc) => _buildDocumentCardCompact(context, doc),
          ),
        ],
  ...
  Widget _buildNoDocumentsCard(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.folder_off_outlined, size: 48, color: Colors.red.shade400),
          const SizedBox(height: 12),
          Text(
            'No Documents Found',
            style: theme.textTheme.titleMedium?.copyWith(
  ...
  Widget _buildEmptyState(BuildContext context, HomeTabState state) {
    ...
    final showAddLoadOffer =
        state.vehicleCount > 0 &&
        state.hasActiveSubscription &&
        state.isAvailableForLoad.toLowerCase() == 'yes';
    final showSubscriptionPayment =
        !state.hasActiveSubscription && state.vehicleCount > 0;

    String emptyStateMessage;
    if (showAddVehicle) {
      emptyStateMessage =
          'Add your vehicle to start offering loads and find customers.';
  ...
  Widget _buildAddVehicleCard(BuildContext context) {
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(14),
        side: BorderSide(color: Colors.green.withValues(alpha: 0.2), width: 1),
      ),
      child: InkWell(
        onTap: () async {
  ...
  void _handleSubscribeNow(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider(
        create: (context) =>
            SubscriptionBloc(getIt<DriverRepository>())
  ...
  void _onPaymentFailure(PaymentFailureResponse response) {
    ...
    _verifyPayment(
      ...
      success: false,
      errorCode:
          errorMeta?['code']?.toString() ?? response.code?.toString() ?? '',
      errorDescription:
          errorMeta?['description']?.toString() ??
